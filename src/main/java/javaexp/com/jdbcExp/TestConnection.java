	...
	public static void main(String[] args) {
		...
		try {
			...
			while (i < 100) {
				try {
					...
					while (rs.next()) {
						if (rs.getString(2).equals("MASTER_SESSION_ID"))
							masterServer = rs.getString(1); // the node that is
															// the master
															// currently has a
															// session named
															// MASTER_SESSION_ID
						else
							slaveServer += (("".equals(slaveServer)) ? "" : ",") + rs.getString(1); // other
																									// nodes
																									// ares
																									// slaves
					}
				...
				System.out
						.println(dateFormat.format(new Date()) + " connected-to:" + connServer + " master:" + masterServer + " slave:" + slaveServer);
				i++;
				try {
					Thread.sleep(1000);
